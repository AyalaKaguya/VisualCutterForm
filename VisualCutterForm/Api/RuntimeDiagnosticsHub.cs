...
namespace VisualMaster.Api
{
    public sealed class RuntimeDiagnosticsHub
    {
        ...
        public RuntimeDiagnosticsHub(int capacity = 500)
        {
        ...
        public void Record(RuntimeDiagnosticEvent diagnosticEvent)
        {
            ...
            lock (_syncRoot)
            {
                _events.AddFirst(diagnosticEvent);
                Trim_NoLock();
            }
        ...
        public IReadOnlyList<RuntimeDiagnosticEvent> GetRecentEvents(int maxCount = 200)
        {
            lock (_syncRoot)
            {
                return _events.Take(Math.Max(1, maxCount)).ToList();
            }
        }
