...
namespace VisualMaster.Api
{
    public sealed class CameraFrameBuffer : IDisposable
    {
        ...
        public long LatestSequenceNumber
        {
            ...
            {
                lock (_syncRoot)
                    return _latestSnapshot?.SequenceNumber ?? 0;
            }
        }
        ...
        public CameraFrameSnapshot WaitForNextSnapshot(long afterSequenceNumber = 0, int timeoutMs = -1)
        {
            lock (_syncRoot)
            {
                ...
                while (!_disposed)
                {
                    ...
                    Monitor.Wait(_syncRoot, Math.Min(remaining, 100));
                    if (_latestSnapshot != null && _latestSnapshot.SequenceNumber > afterSequenceNumber)
                        return _latestSnapshot.AddRef();
                }
        ...
        {
            if (_disposed)
                throw new ObjectDisposedException(nameof(CameraFrameBuffer));
        }
    }
}